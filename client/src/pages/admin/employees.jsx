...
export default function employees() {
    ...
    const filterUsersByOrder = (str) => {
        ...
        if (str === "username") {
            ...
            if (!order.phone) {
                setOrder({ ...order, phone: true });
                sortedUsers = filteredUsers.slice().sort((a, b) => {
                    return b.phone.localeCompare(a.phone); 
                });
            } else {
                setOrder({ ...order, phone: false });
                sortedUsers = filteredUsers.slice().sort((a, b) => {
    ...
    const deleteUserHandle = async (employeeIds) => {
        try {
            const response = await axiosClient.delete(`/employee`, {
                data: { employeeIds } 
            });
             return true;
        } catch (error) {
  ...
  return (
    ...
    <div className="flex w-full items-center justify-between pt-5">
        <h4 className='text-4xl font-semibold text-[#141414]'>Our Employees</h4>
        <button 
            className='py-2 px-5 rounded-lg bg-[#356fc6] text-white text-lg '
          >
            <Link to="/admin/employees/add">new employee</Link>
        </button>
