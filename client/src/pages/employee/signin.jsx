...
export default function SignIn() {
  ...
  const sendData = async (formData) => {
    try {
      ...
      const response = await axiosClient.post('http://localhost:5555/employee/signin', { email, password});
      if (response.status === 200) {
        localStorage.setItem('token', response.data.token);
        localStorage.setItem('employee', true);
        userContext.getEmployee();
    ...
    <div className="max-w-6xl  mx-auto h-screen">
      ...
      <ToastContainer />
       ...
       <div className='basis-2/2 md:basis-1/2 w-full mx-16 flex flex-col lg:mr-10 py-24 lg:pb-0  '>
         <div> 
           <h2 className='text-5xl font-bold mb-10 text-center text-[#414141]'>Welcome Back</h2>
           <div className=' mb-3'>
